...
[Chzzk / Cime API] ──WebSocket──▶ [Tauri Backend (Rust)]
                ...
                [OS 화면리더 → 점자 단말기]         Linux/Windows: Qwen3-30B-A3B GGUF (mistral.rs, CPU)
                                                          │
                                                          ▼
                                                  [요약 텍스트 → live region]
```
...
- **On-device LLM** — iOS: Apple Foundation Models / Android: Gemini Nano (AICore) / Linux · Windows: Qwen3-30B-A3B GGUF via [mistral.rs](https://github.com/EricLBuehler/mistral.rs) (CPU 전용)
...
- Linux · Windows에서 데스크톱용 LLM 요약을 쓰려면 GGUF 모델 파일 별도 다운로드 (아래 섹션 참조)

## 데스크톱 LLM (Linux / Windows)

iOS는 Apple Foundation Models, Android는 Gemini Nano (AICore)로 OS 내장 LLM에 위임한다. Linux와 Windows에서는 OS 내장 LLM이 없으므로 [mistral.rs](https://github.com/EricLBuehler/mistral.rs) 라이브러리로 **Qwen3-30B-A3B** (활성 파라미터 3B MoE) GGUF 모델을 CPU에서 직접 추론한다.

3가지 플랫폼 모두 `LlmSummarizer` trait의 단일 인터페이스로 노출되므로 호출 측 코드는 동일하다. mistral.rs 의존성은 `[target.'cfg(any(target_os = "linux", target_os = "windows"))'.dependencies]`로 묶여 모바일 빌드에는 들어오지 않는다.
...
Windows는 같은 명령을 PowerShell에서 실행하고 경로만 `%LOCALAPPDATA%\jeomjwabot\models\`로 바꿔주면 된다.

| 양자화 | 파일 크기 | 권장 RAM | 비고 |
|---|---|---|---|
| UD-IQ2_M | 10 GB | 16 GB | 품질 손실 큼 — 한국어 깨짐 위험 |
| UD-Q3_K_XL | 13 GB | 24 GB | 절충안 |
| **UD-Q4_K_XL** | **16.5 GB** | **32 GB+** | **기본 권장** (unsloth 동적 양자화) |
| UD-Q5_K_XL | 20 GB | 40 GB+ | 여유 있을 때 |

CPU(8~16코어, AVX2 가정) 기준 Q4 양자화에서 5~15 tok/s. 점좌봇은 N초 간격 짧은 요약이라 충분.

### 모델 경로 결정

런타임에 다음 순서로 결정된다.

1. `JEOMJWABOT_MODEL_DIR` 환경변수 (있으면 우선)
2. `dirs::cache_dir().join("jeomjwabot/models")` — Linux는 `$HOME/.cache/jeomjwabot/models`, Windows는 `%LOCALAPPDATA%\jeomjwabot\models\`

파일명은 기본 `Qwen3-30B-A3B-UD-Q4_K_XL.gguf`. 다른 양자화로 받았으면 `JEOMJWABOT_MODEL_FILE` 환경변수로 오버라이드.

### 실행

다운로드가 끝나고 파일이 위 경로에 있으면 별도 환경변수 없이 그대로 실행:

```bash
cargo tauri dev
```

앱 시작 직후 백그라운드에서 모델을 1회 로드한다(30초~3분, 디스크 속도 의존). 로드 진행은 로그에서 확인:
...
로드가 끝나기 전에 요약 IPC가 호출되면 명시적인 한국어 에러("요약 모델이 아직 준비되지 않았습니다.")로 즉시 실패시켜, 가짜 요약이 점자단말기로 흘러가지 않게 한다.

### 모델 파일이 없으면

앱 자체는 정상 실행되며, 요약을 시도할 때만 `IpcError::MissingConfig`로 실패한다. 화면리더는 한국어로 "Qwen GGUF 파일을 찾을 수 없습니다: ..." 안내를 읽는다. 다운로드 후 앱 재시작.

### Qwen3.6 호환성 노트

원래 목표 모델은 **Qwen3.6-35B-A3B** (2026-04-16 출시) 이지만 현재 mistral.rs (v0.8.x · master) 의 GGUF 로더는 그 GGUF 파일이 선언하는 `qwen35moe` 아키텍처를 지원하지 않는다. 텐서 레이아웃이 Qwen3 MoE와 달라(post-norm · 게이트 어텐션 · 공유 전문가 · SSM 하이브리드 블록 추가) 단순한 enum 패치로는 안 풀리고 별도 로더 작성이 필요하다.

따라서 임시로 직전 세대인 **Qwen3-30B-A3B**로 fallback. 같은 활성 3B MoE이고 한국어 짧은 요약 품질에 큰 차이 없음. mistral.rs upstream에 `qwen35moe` 지원이 들어오면 다운로드 파일과 `JEOMJWABOT_MODEL_FILE` 환경변수만 바꿔 다시 Qwen3.6으로 전환할 수 있다.
