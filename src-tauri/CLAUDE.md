...
- Cime: Access Token (Bearer).
- 토큰은 OS keyring에 저장 (`tauri-plugin-stronghold` 또는 `keyring` 크레이트). **plain text 파일/JSON에 저장 금지**.
- Refresh token 만료 처리 → 재로그인 UI emit.

## 디바이스 LLM 호출
- iOS: `Foundation Models`는 Swift. `swift-bridge` 또는 Tauri 플러그인의 `MobileBuilder` 훅으로 호출.
- Android: AICore의 Gemini Nano는 Java/Kotlin. JNI 또는 `tauri-plugin-android` 패턴.
- Rust 진입점: `trait LlmSummarizer`. iOS/Android/desktop-mock을 `#[cfg(target_os = ...)]`로 분기.
