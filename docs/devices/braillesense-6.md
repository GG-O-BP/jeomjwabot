...
한소네 6는 점자 출력만 하는 디스플레이가 아니라 **양방향 노트테이커**다. 외부 디스플레이 모드(길 A)에서도 단말기 자체의 키 입력은 OS 키 이벤트로 호스트에 전달되므로, 점좌봇 UI는 단말기 키만으로 조작 가능하다.

| 컨트롤 | 구성 | 점좌봇 매핑 후보 |
|---|---|---|
| 퍼킨스식 점자 키보드 | 6점 입력 6키 + 스페이스바 + 백스페이스 + 엔터 | 채팅 입력, 텍스트 검색 |
| Ctrl, Alt | modifier | OS 화면리더 단축키와 공존 |
| F1~F4 | 멤브레인 기능키 4개 | 필터 토글·모드 전환 등 자유 매핑 |
| 미디어 키 | 재생/정지/이전/다음 | 자동 출력 일시정지·재개, "이전/다음 요약" |
| 볼륨 키 | OS 시스템 볼륨 | (단말 차원, 앱 무관) |
| 좌우 스크롤 키 | 점자 라인 좌/우 페이징 | 이전/다음 요약 페이지 |
| 커서 라우팅 키 | 32셀 위 32개 — 특정 셀 위치로 즉시 점프 | **채팅 입력 오타 수정·인용 — 시각 사용자 마우스 클릭 동급 속도** |
| 잠금 스위치 | 키 입력 잠금 | (단말 차원) |
| 모드 스위치 | 자체 노트테이커 ↔ Terminal for Screen Reader | 길 A 진입/이탈 트리거 |
| 파워 버튼 | 단말기 전원 | (단말 차원) |

> 커서 라우팅 키의 존재 때문에 "점자 입력은 느리다"는 가정은 틀리다. 32셀 라인 어디든 1tap으로 캐럿 점프가 된다.

좌우 스크롤·미디어·F키·Ctrl/Alt 조합은 모두 OS 키 이벤트로 점좌봇에 도달한다 → `aria-keyshortcuts`로 의미 있는 매핑을 노출하면 단말기 키만으로 앱 전체 조작이 가능하다.
